...
export const renderDecks = (root, go) => {
  ...
  const paint = () => {
    ...
    list.innerHTML = decks.map((d) => {
      ...
      return `<article class="card"><div class="row"><strong>${d.name}</strong><span>${count} tarjetas</span></div><div class="row"><button class="btn" data-enter="${d.id}">Entrar</button><button class="btn-ghost" data-edit="${d.id}">✏️</button><button class="btn-ghost" data-delete="${d.id}">🗑️</button></div></article>`;
    }).join('');
  };
  paint();
  ...
  list.onclick = (e) => {
    ...
    if (e.target.dataset.edit) {
      ...
      if (!next?.trim()) return;
      saveDecks(getDecks().map((d) => (d.id === id ? { ...d, name: next.trim() } : d)));
    }
