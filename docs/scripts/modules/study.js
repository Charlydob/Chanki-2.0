...
import { getNextPhrase, markPhraseReviewed } from '../phrases.service.js';

const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
const buildStudyPool = () => getCards().filter((card) => card.exploreStatus !== 'known').map((card) => ({ ...card, source: card.source || 'card' }));
...
export const renderStudy = async (root) => {
  ...
  root.querySelector('#flash').onclick = (e) => { if (e.target.closest('button') || e.target.closest('input')) return; showingBack = !showingBack; paint(); };
  ...
  root.querySelectorAll('[data-phrase]').forEach((btn) => btn.onclick = () => { markPhraseReviewed(phrase.id, btn.dataset.phrase); saveProgress([...getProgress(), { cardId: `phrase-${phrase.id}`, grade: btn.dataset.phrase === 'fail' ? 'fail' : btn.dataset.phrase === 'known' ? 'known' : 'doubt', at: Date.now(), source: 'phrase' }]); });
};
